...
MYIP=$(cat /usr/bin/.ipvps)
IZIN_FILE="/root/izin/ip"
eval $(curl -qO- "https://raw.githubusercontent.com/nameless-newbie/fifth-sc/main/izin/ip")
...
if [[ -f /etc/os-release ]]; then
    ...
    echo "Operation Systems found: $OS_NAME $OS_VERSION"
else
    echo "Couldn't find operation system."
    exit 1
fi
if [[ -e /etc/xray/domain ]]; then
domain=$(cat /etc/xray/domain)
else
domain="kuzavpn.cloud"
fi
...
if [[ "$OS_NAME" == "debian" && "$OS_VERSION" == "10" ]] || [[ "$OS_NAME" == "ubuntu" && "$OS_VERSION" == "20.04" ]]; then
    echo "Installing squid3 for Debian 10 or Ubuntu 20.04..."
    apt -y install squid3
else
    echo "Installing squid for other versions..."
    apt -y install squid
fi
...
echo "Replacing the IP placeholder with the current IP address..."
sed -i $MYIP2 /etc/squid/squid.conf

echo "The installation and configuration of Squid is complete."
# setting vnstat
